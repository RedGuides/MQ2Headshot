...
#include "ISXEQHeadshot.h"

// The mandatory pre-setup function.  Our name is "ISXEQHeadshot", and the class is ISXEQHeadshot.
...
// our instanced class.
ISXPreSetup("ISXEQHeadshot",ISXEQHeadshot);
...
unsigned int ISXEQHeadshotXML=0;
...
bool ISXEQHeadshot::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQHeadshot Loaded");
return true;
}

// shutdown sequence
void ISXEQHeadshot::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQHeadshotXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQHeadshotXML);
...
void ISXEQHeadshot::ConnectServices()
{
...
void ISXEQHeadshot::RegisterCommands()
{
    // add any commands
...
void ISXEQHeadshot::RegisterAliases()
{
    // add any aliases
}

void ISXEQHeadshot::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQHeadshot::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQHeadshot::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQHeadshot::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQHeadshot::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQHeadshot::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQHeadshotService)
    //        pISInterface->ShutdownService(this,hISXEQHeadshotService);
}

void ISXEQHeadshot::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQHeadshotXML
    ISXEQHeadshotXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQHeadshotXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQHeadshotXML,"General"))
    {
